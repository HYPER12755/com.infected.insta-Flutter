...
class _NotificationList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<Map<String, dynamic>>>(
      stream: repo.getNotifications(userId).map((r) =>
          r.fold((_) => <Map<String, dynamic>>[], (n) => n)),
      builder: (context, snap) {
        ...
        if (notifs.isEmpty) {
          return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center,
            children: [
              ...
              Text('When people like or comment on your posts, you\'ll see it here.',
        ...
        for (final n in notifs) {
          ...
          if (d.inHours < 24) today.add(n);
          else if (d.inDays < 7) thisWeek.add(n);
          else earlier.add(n);
        }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: ListTile(
        ...
        leading: Stack(children: [
          CircleAvatar(
            ...
            backgroundImage: actorAvatar.isNotEmpty
                ? NetworkImage(actorAvatar) as ImageProvider : null,
            child: actorAvatar.isEmpty
                ? Text(actor.isNotEmpty ? actor[0].toUpperCase() : 'U',
        ...
        subtitle: Text(_fmtTime(notif['created_at']),
        ...
        trailing: postImage.isNotEmpty
            ...
            : (type == 'follow' ? _FollowBackBtn(
                userId: notif['actor_id'] as String? ?? '') : null),
        onTap: () {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () async {
        ...
        if (_following) {
          await supabase.from('follows').upsert({
            'follower_id': uid, 'following_id': widget.userId});
        } else {
          await supabase.from('follows').delete()
              .match({'follower_id': uid, 'following_id': widget.userId});
        }
      },
      child: AnimatedContainer(
        ...
        child: Text(_following ? 'Following' : 'Follow Back',
            style: TextStyle(
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await supabase
          ...
          .select('id, requester_id, profiles!follow_requests_requester_id_fkey(username, avatar_url)')
          .eq('target_id', uid)
          .eq('status', 'pending');
    ...
    final content = _isLoading
        ...
        : _requests.isEmpty
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const FaIcon(FontAwesomeIcons.userClock,
